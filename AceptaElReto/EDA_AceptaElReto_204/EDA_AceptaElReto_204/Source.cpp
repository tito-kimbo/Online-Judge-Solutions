...
inline bintree<char> leerArbol() {
	char raiz;
	std::cin >> raiz;
	if (!std::cin) { // es un árbol vacío
		return{};
	}
	else if (raiz == '*' || raiz == '.') {
		return { raiz };
...
void show(const bintree<char> & t) {
	if (!t.empty()) {
		std::cout << t.root();
		show(t.left());
		show(t.right());
...
bool bienDecorado(const bintree<char> & bt, int & nBolas){
	bool isOK = true;
	int bIzq, bDer;

	if (bt.empty()) return true;

	bIzq = bDer = 0;
	isOK = bienDecorado(bt.left(), bIzq) && bienDecorado(bt.right(), bDer);
	if (isOK) {
		nBolas = bIzq + bDer;
		if (bt.root() == '*') nBolas++;
		isOK = std::abs(bIzq - bDer) <= 1;
	}
	return isOK;
}
