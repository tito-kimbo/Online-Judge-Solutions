...
public:
	...
	//ADD
	void add(const bintree<T> & bt) {
		int e = 0;
		bintree<T> aux1, aux2;
		if (!bt.empty()) {
			if (empty()) {
				*this = bintree<T>(bintree<T>(), bt.root(), bintree<T>());
				e = bt.root();
			} else {
				e = raiz->elem + bt.root();
			}
...
std::ostream & operator<<(std::ostream & out, const bintree<int> & bt) {
	out << bt.root();
	if (!bt.left().empty()) out << ' ' << bt.left();
	else out << ' ' << -1;
	if (!bt.right().empty()) out << ' ' << bt.right();
	else out << ' ' << -1;
